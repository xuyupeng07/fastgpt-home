...
export default function FormModal() {
  ...
  return (
    <div
      ...
      } ${isFormModalOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
    >
      ...
      <div
        className={`relative flex h-[94dvh] w-full flex-col overflow-hidden rounded-t-lg bg-white shadow-2xl transition-all duration-300  sm:h-[min(90dvh,820px)] sm:max-w-[760px] sm:rounded-lg ${
          isFormModalOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'
        }`}
      >
        {/* 弹窗头部（紧凑版式，给 iframe 让出高度） */}
        <header className="relative shrink-0 border-b border-[#eaecf0] bg-white px-5 py-3.5 pr-14   sm:px-7 sm:py-4 sm:pr-16">
          <span className="mb-1 block text-[11px] font-semibold uppercase text-brand-600 ">
            企业服务
          </span>
          <h2
          ...
          <p className="mt-0.5 max-w-[570px] truncate text-xs leading-5 text-[#667085] ">
        ...
        {renderOverlay && (
          <div className="relative min-h-0 flex-1 w-full bg-[#f7f8fa] ">
            {isIframeLoading && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4 bg-white ">
                <div className="relative">
                  ...
                  <div className="absolute top-0 left-0 w-12 h-12 border-4 border-brand-600  rounded-full border-t-transparent animate-spin" />
                </div>
