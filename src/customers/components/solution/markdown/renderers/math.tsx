...
const MathFormula = lazy(async () => {
  ...
  return {
    default: function MathFormula({
      ...
      if (!html) {
        return block ? (
          <pre className="not-prose my-4 overflow-x-auto rounded-xl bg-slate-100 p-4 text-sm ">
            {formula}
          </pre>
        ) : (
          <code>{formula}</code>
      ...
      if (block) {
        return (
          <div
            className="not-prose scrollbar-hidden my-6 overflow-x-auto rounded-2xl border border-slate-200/80 bg-slate-50/80 px-4 py-5  "
...
export const mathMarkdownRenderers: Components & Record<string, unknown> = {
  ...
  'math-inline': (props: { node?: unknown } & MarkdownRendererProps) => {
    ...
    return (
      <Suspense fallback={<code>{formula}</code>}>
        <MathFormula formula={formula} displayMode={props['data-display'] === 'true'} />
      </Suspense>
    );
  },
