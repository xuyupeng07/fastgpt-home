...
const GITHUB_STARS_LABEL = '29,429+';
...
export default function Hero({
  ...
  // GitHub 星标已在顶部徽章展示，统计条里不再重复。
  const stats = overviewStats.filter(
    (item) => !item.label.includes('GitHub') && !item.label.includes('星标')
  );
        ...
        <div className="relative max-w-[min(92vw,1300px)] mx-auto flex flex-col items-center text-center gap-[50px] md:gap-[32px] px-[16px] md:px-[32px]">
          ...
          >
            ...
            <span className="text-[12px] leading-[18px]" style={{ color: 'rgb(71, 85, 105)' }}>
              Github Stars {GITHUB_STARS_LABEL}
            </span>
            <a
              href={GITHUB_URL}
              target="_blank"
          ...
          <m.p
            ...
            style={{ color: '#4b5563' }}
          >
            依托 FastGPT 工作流引擎与知识库检索能力，我们将高频业务场景沉淀为标准化、可验证、可交付的
            AI 解决方案，帮助企业从场景评估、POC 验证到生产环境上线高效落地。
          </m.p>
